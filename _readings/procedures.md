...
summary: |
  We explore why and how to define your own functions in Scheme.
prereqs: |
  [Algorithm building blocks]({{ "/readings/algorithm-building-blocks.html" | relative_url }}).
  [An abbreviated introduction to Scheme]({{ "/readings/intro-scamper.html" | relative_url }}).
preimg: true
---
...
For example, we might want to define a function, `square`, that takes as input a number and computes the square of that number.
...
consider an example. (Although our functions are called `solid-square`
...
As we noted, the `csc151` library already defines a `square` function, so it's unlikely to be a good idea for us to define our own `square` function, whether for numbers or images.
...
So, how do we define these functions?  Read on and see.

## Defining functions with `lambda`

Scheme provides a variety of mechanisms for defining functions.
We will start with the most general, which uses the keyword `lambda`, which means "function".
The lambda mechanism is relatively straightforward.

As you may recall, we typically think of a function as having three main aspects: The _name_ we use to refer to the function, the names of the _parameters_ (inputs) to the function, and the _instructions_ the function executes.

Here is the general form of function definitions in Scheme, at least as we will use them in this class.
...
In this case, we're naming a function, rather than a value.

The _function-name_ part is straightforward; it's the name we will use to refer to the function.
You can use any valid identifier as the function name.
The "`lambda`" is a special keyword in Scheme to indicate that "_Hey! This is a function!_".
...
For example, we might use the names `side-length` and `color` for the inputs to our "make a square of a certain color" function.  Similarly, we might use the name `x` for the input to the "square a number" function.
...
> `square-number` names a function that takes one input, `x`, and computes
its result by multiplying `x` by itself.

While you will normally define function in the definitions pane, you can also create them in the interactions pane.  Let's see how this approach works.
...
You may note in the last line that when we asked our Scheme interpreter for the value of `square-number`, it told us that it's a function by giving us back a `lambda`.
...
And, in some cases, it's a function.

How does the function we've just defined work?
Here's one way to think about it: When you call a function you've defined with `lambda`, the Scheme interpreter substitutes in the arguments in the function call for the corresponding parameters within the instructions.
...
If we want a function to make squares, we'll just call the `rectangle` function, using the same value for the width and height.
...
> (overlay/align "center" "bottom"
                 ...
                 (above (solid-isosceles-triangle 50 30 "red")
...
We will replace each `50` by `size` and replace `40` by `(* 0.8 size)`.
...
The next step might be to add parameters for the color of the body, the color of the roof, and perhaps even the slope of the roof.
...
Runtime error · line 3, column 3
Selection deleted
```

Don't worry about how to make the error show up; it will happen naturally.
...
It was updated again in Fall 2026 as we transitioned to Racket and revisited the terminology we use in the course.
