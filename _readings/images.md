...
preimg: true
---
_Disclaimer: The Scamper image library does not quite match its specifications. Expect a bit of unexpected behavior. Don't worry! Things will be consistent in about a week_

## Introduction

In addition to supporting "standard" data types, such as numbers and strings, Scamper also includes libraries that support a number of more sophisticated data types, including a type that the designers call "images".  The image data type supports the creation, combination, and manipulation of a variety of basic shapes.  Readers of an earlier generation might consider Scamper's image type an extension of the ColorForms that they played with as children.
...
How does Scamper _display_ these values?
...
(The `pen-width` parameter currently doesn't work. The examples show what things would look like if it worked correctly.)
...
There are also ellipses, squares, equilateral triangles, isosceles triangles, and even generalized polygons, each in a solid and an outlined form.
...
Polygons are a bit more complicated (and currently unavailable). We won't discuss all the details yet, but a few examples might be of interest. Note that `(pt x y)` creates an x/y point on an upside-down coordinate system.
...
What if we don't want things aligned on centers?  The Scamper imge library provides alternatives to these three that provide a bit more control.
...
You can (eventually) find information on more ways to combine images in the help system.
...
In fact, it may not only be more accurate to represent colors non-textually, it may also be more _efficient_, since color names may require the computer to look up the name in a table.
...
In Scamper's image model, you can use the `rgb` procedure to create RGB colors.  `(rgb 0 255 0)` makes a bright green, `(rgb 0 128 128)` makes a blue-green color, and `(rgb 64 0 64)` makes a relatively dark purple.

```drracket
> (beside (solid-circle 40 (rgb 0 255 0))
          (solid-circle 40 (rgb 0 128 128))
          (solid-circle 40 (rgb 64 0 64)))
...
> (above
    (overlay circles (solid-rectangle 120 40 (rgb 0 0 255 255)))
    (overlay circles (solid-rectangle 120 40 (rgb 0 0 255 191)))
    (overlay circles (solid-rectangle 120 40 (rgb 0 0 255 127)))
    (overlay circles (solid-rectangle 120 40 (rgb 0 0 255  63))))
...
Even though we no longer use the HtDP library or even Racket, this section draws upon [The DrRacket HtDP/2e Image Guide](https://docs.racket-lang.org/teachpack/2htdpimage.html).  
